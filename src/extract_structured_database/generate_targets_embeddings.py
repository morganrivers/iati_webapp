...
def main() -> None:
    ...
    print(f"Found {len(existing_ids)} existing embeddings in {OUT_PATH}")
    ...
    with OUT_PATH.open("a", encoding="utf-8") as out_f:
        for processed, (aid, section, text) in enumerate(iter_targets(INPUT_PATH), start=1):
            ...
            print("record (embedding 'text' from that)")
            print('record["activity_id"]')
            print(record["activity_id"])
            print('record["model"]')
            print(record["model"])
            ...
            if new_count % 25 == 0:
                print(f"Embedded {new_count} new (processed {processed}, skipped existing {skipped})")

    print(f"Done. Processed: {processed} | New embeddings: {new_count} | Skipped existing: {skipped}")
