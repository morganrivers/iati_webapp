# Loads ../../data/outputs_misc.jsonl, converts loan/disbursement to USD (base units),
# skipping zeros/""/None/-1 (sentinels), printing failures, and plotting histograms.
#
# Updates vs prior:
# - Clean currency detection (no duplicate junk), handles: €, £, US $, $USD, USDollars, U$S
# - Handles "thousand(s)" / "k" units (e.g., "US$ thousands")
# - Handles "($m.)", "$ Million", "million $", "million £", "US dollars million"
# - If unit is just "million"/"thousand" with no currency, tries to infer currency from the other field in the same record
# - Treats verbose "Not specified..." / "Units not specified" / "No disbursements..." as missing
# - Adds MXN, PEN, BRL FX placeholders (approx). UA/UC/SDR still unsupported unless you wire rates.
...
if __name__ == "__main__":
    loans_x, loans_label, disb_x, disb_label = get_loans_disbursements()
    plot_histograms_loans_disbursements(loans_x, loans_label, disb_x, disb_label, BINS=BINS)
# # Loads ../../data/outputs_misc.jsonl, converts loan/disbursement to USD (base units),
# # skipping zeros/""/None/-1 (sentinels), printing failures, and plotting histograms.
# #
# # Updates vs prior:
...
# # Heuristic: rescale extreme values by 1e6
# RESCALE_EXTREMES_BY_MILLION = False
# LOW_USD_THRESHOLD = 10_000          # if < $10k, treat as "missing million" => * 1e6
# HIGH_USD_THRESHOLD = 10_000_000_000 # if > $10B, treat as "over-applied million" => / 1e6
...
#     # load_forecast_meta: returns (scope_by_aid, planned_by_aid, actual_by_aid)
#     _, planned_by_aid, _ = load_forecast_meta(Path(FORECAST_CSV))

#     # load_best_loan_disb: returns (best_loan, best_disb, n_lines, n_outer_bad, n_inner_bad)
#     best_loan, best_disb, n_lines, n_outer_bad, n_inner_bad = load_best_loan_disb(Path(OUT_MISC))

#     aids = set(best_disb) | set(planned_by_aid) | set(best_loan)
...
#             # "planned_loan": [get_loan(aid) for aid in aids],
#         }
